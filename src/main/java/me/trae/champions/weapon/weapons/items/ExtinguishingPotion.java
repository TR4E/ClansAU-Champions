...
public class ExtinguishingPotion extends ActiveCustomItem<Champions, WeaponManager, WeaponData> implements Listener {
    ...
    private void onLeftClick(final Player player) {
        final Throwable throwable = new Throwable(this.getAbilityName(ActionType.LEFT_CLICK), this.getItemStack(), player, 10_000L, player.getEyeLocation(), player.getEyeLocation().getDirection().multiply(this.itemVelocity));
